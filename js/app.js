 ...
 */
    ...
    (() => {
        ...
        function showMore() {
            window.addEventListener("load", (function(e) {
                ...
                function initItem(showMoreBlock, matchMedia = false) {
                    ...
                    if (showMoreBlock.dataset.showmoreItems) {
                        const showMoreItems = showMoreContent.children;
                        const showMoreItemsCount = parseInt(showMoreBlock.dataset.showmoreItems);
                        for (let i = 0; i < showMoreItems.length; i++) if (i >= showMoreItemsCount) showMoreItems[i].hidden = true; else showMoreItems[i].hidden = false;
                        showMoreButton.hidden = showMoreItems.length <= showMoreItemsCount;
                        showMoreButton.addEventListener("click", (function() {
                            if (showMoreBlock.classList.contains("_showmore-active")) {
                                for (let i = 0; i < showMoreItems.length; i++) if (i >= showMoreItemsCount) showMoreItems[i].hidden = true;
                                showMoreBlock.classList.remove("_showmore-active");
                            } else {
                                for (let i = 0; i < showMoreItems.length; i++) showMoreItems[i].hidden = false;
                ...
                function getHeight(showMoreBlock, showMoreContent) {
                    ...
                    if (showMoreType === "items") {
                        ...
                        const showMoreTypeValue = showMoreContent.dataset.showmoreContent ? showMoreContent.dataset.showmoreContent : 190;
                ...
                function showMoreActions(e) {
                    ...
                    if (targetType === "click") {
                        if (targetEvent.closest("[data-showmore-button]")) {
                            ...
                            if (showMoreBlock.dataset.showmoreItems) return;
        ...
        if (inputMasks.length) modules_flsModules.inputmask = Inputmask("+380 (999) 999-99-99").mask(inputMasks);
        var PipsMode;
        (function(PipsMode) {
        ...
        function rangeInit() {
            ...
            if (priceSlider) {
                ...
                function validateNumberInput(e) {
                    if (!/[\d,]/.test(e.key) && e.key !== "Backspace" && e.key !== "Delete" && e.key !== "Tab" && e.key !== "Escape" && e.key !== "Enter") e.preventDefault();
                    if (e.key === "," && e.target.value.includes(",")) e.preventDefault();
                }
                priceStart.addEventListener("keydown", validateNumberInput);
                priceEnd.addEventListener("keydown", validateNumberInput);
                function formatInputOnBlur(e) {
                    if (e.target.value !== "") {
                        const cleanValue = e.target.value.replace(/[^\d,]/g, "");
                        const numValue = parseInt(cleanValue.replace(/,/g, ""));
                        if (!isNaN(numValue)) e.target.value = formatNumber(numValue); else e.target.value = "";
                    }
                }
                priceStart.addEventListener("blur", formatInputOnBlur);
                priceEnd.addEventListener("blur", formatInputOnBlur);
                priceSlider.noUiSlider.on("update", (function(values, handle) {
                    if (handle) priceEnd.value = values[handle]; else priceStart.value = values[handle];
                }));
                ...
                function setPriceValues() {
                    let priceStartValue;
                    let priceEndValue;
                    if (priceStart.value != "") priceStartValue = unformatNumber(priceStart.value);
                    if (priceEnd.value != "") priceEndValue = unformatNumber(priceEnd.value);
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                new swiper_core_Swiper(".main-banner__slider", {
                ...
                new swiper_core_Swiper(".catalog-page__categories-list", {
                    modules: [ Navigation, Pagination, Autoplay ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
                ...
                let serviceOverviewThumbs = new swiper_core_Swiper(".service-overview__slider-thumbs .slider-thumb", {
                    ...
                    speed: 800,
                    pagination: {
                        el: ".service-overview .swiper-pagination",
                        clickable: true
                    },
                    navigation: {
                        prevEl: ".service-overview .swiper-button-prev",
                        nextEl: ".service-overview .swiper-button-next"
                    },
                    breakpoints: {
                        320: {
                            slidesPerView: "auto",
                ...
                new swiper_core_Swiper(".service-overview__slider", {
                    ...
                    navigation: {
                        prevEl: ".service-overview__navigation .swiper-button-prev",
                        nextEl: ".service-overview__navigation .swiper-button-next"
                    },
                    on: {
                        init: function() {
                        ...
                        slideChange: function() {
                            updateBackgroundImage(this.activeIndex);
                            const slides = document.querySelectorAll(".service-overview__slider .swiper-slide");
                ...
                const productThumbsElement = document.querySelector(".product-page__slider-thumb");
                const productSliderElement = document.querySelector(".product-page__slider");
                let productPageThumbs;
                if (productThumbsElement) productPageThumbs = new swiper_core_Swiper(".product-page__slider-thumb", {
                    modules: [ Thumb ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: "auto",
                ...
                if (productSliderElement) {
                    new swiper_core_Swiper(".product-page__slider", {
                        modules: [ Thumb ],
                        observer: true,
                        autoHeight: true,
                        observeParents: true,
                        ...
                        spaceBetween: 0,
                        speed: 800,
                        thumbs: productPageThumbs ? {
                            swiper: productPageThumbs
                        } : null
                    });
                }
                new swiper_core_Swiper(".main-popular-products__slider.slider-1", {
                    modules: [ Navigation, Pagination, Autoplay ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 3.2,
                    ...
                    speed: 800,
                    pagination: {
                        el: ".main-popular-products__slider.slider-1 .swiper-pagination",
                        clickable: true
                    },
                    navigation: {
                        prevEl: ".main-popular-products__slider.slider-1 .swiper-button-prev",
                        nextEl: ".main-popular-products__slider.slider-1 .swiper-button-next"
                    },
                    breakpoints: {
                        320: {
                            slidesPerView: 1.2,
                ...
                new swiper_core_Swiper(".main-popular-products__slider.slider-2", {
                    modules: [ Navigation, Pagination, Autoplay ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 3.2,
                    ...
                    speed: 800,
                    pagination: {
                        el: ".main-popular-products__slider.slider-2 .swiper-pagination",
                        clickable: true
                    },
                    navigation: {
                        prevEl: ".main-popular-products__slider.slider-2 .swiper-button-prev",
                        nextEl: ".main-popular-products__slider.slider-2 .swiper-button-next"
                    },
                    breakpoints: {
                        320: {
                            slidesPerView: 1.2,
                ...
                new swiper_core_Swiper(".main-popular-products__slider.slider-3", {
                    modules: [ Navigation, Pagination, Autoplay ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 3.2,
                    ...
                    speed: 800,
                    pagination: {
                        el: ".main-popular-products__slider.slider-3 .swiper-pagination",
                        clickable: true
                    },
                    navigation: {
                        prevEl: ".main-popular-products__slider.slider-3 .swiper-button-prev",
                        nextEl: ".main-popular-products__slider.slider-3 .swiper-button-next"
                    },
                    breakpoints: {
                        320: {
                            slidesPerView: 1.2,
                ...
                new swiper_core_Swiper(".main-popular-products__slider.slider-4", {
                    modules: [ Navigation, Pagination, Autoplay ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 3.2,
                    ...
                    speed: 800,
                    pagination: {
                        el: ".main-popular-products__slider.slider-4 .swiper-pagination",
                        clickable: true
                    },
                    navigation: {
                        prevEl: ".main-popular-products__slider.slider-4 .swiper-button-prev",
                        nextEl: ".main-popular-products__slider.slider-4 .swiper-button-next"
                    },
                    breakpoints: {
                        320: {
                            slidesPerView: 1.2,
        ...
        function updateBackgroundImage(index) {
            const backgroundItems = document.querySelectorAll(".service-overview__backgrounds-item");
            if (backgroundItems && backgroundItems.length) backgroundItems.forEach(((item, i) => {
        ...
        function digitsCounter() {
            function digitsCountersInit(digitsCountersItems) {
                ...
                if (digitsCounters.length) digitsCounters.forEach((digitsCounter => {
                    ...
                    digitsCounter.dataset.digitsCounter = clearNumber(digitsCounter.innerHTML) || digitsCounter.dataset.digitsCounter;
            ...
            function digitsCountersAnimate(digitsCounter) {
                ...
                const duration = parseFloat(digitsCounter.dataset.digitsCounterSpeed) || 1e3;
                const startValue = clearNumber(digitsCounter.dataset.digitsCounter) || 0;
                const format = digitsCounter.dataset.digitsCounterFormat || "";
                const step = timestamp => {
                    if (!startTimestamp) startTimestamp = timestamp;
                    const progress = Math.min((timestamp - startTimestamp) / duration, 1);
                    const value = Math.floor(progress * startValue);
                    digitsCounter.innerHTML = formatNumber(value, format);
            ...
            function formatNumber(value, format) {
                if (format === "comma") return value.toLocaleString("en-US"); else if (format === "space") return value.toLocaleString("ru-RU");
                return value;
            }
            document.addEventListener("DOMContentLoaded", (() => digitsCountersInit()));
        }
        setTimeout((() => {
...
***************************************************************************** */
        ...
        da.init();
        const dropdownsEvents = () => {
            const dropdowns = document.querySelectorAll('[data-target="dropdown"]');
            if (dropdowns.length) dropdowns.forEach((dropdown => {
                const button = dropdown.querySelector("[data-button]");
        ...
        const headerMenuEvents = () => {
            ...
            const headerMenuPC = document.querySelector(".header-menu-pc");
            const headerMenuPCMenus = document.querySelectorAll(".header-menu-pc__menu");
            const menuButtons = document.querySelectorAll(".menu-button");
            const catalogButton = document.querySelector(".menu-button-catalog");
            const catalogMenu = document.getElementById("catalog-menu");
            if (!headerMenuPC || !headerMenuPCMenus.length) return;
            headerMenuPCMenus.forEach((menu => {
                const listItems = menu.querySelectorAll(".menu-pc__list ul li");
                listItems.forEach(((item, index) => {
                    item.style.setProperty("--item-index", index);
                }));
            }));
            const closeAllMenus = () => {
                if (menuItems && menuItems.length) menuItems.forEach((item => item.classList.remove("active")));
                headerMenuPCMenus.forEach((menu => menu.classList.remove("active")));
                headerMenuPC.classList.remove("active");
                if (catalogMenu) catalogMenu.classList.remove("active");
            };
            if (menuItems && menuItems.length) menuItems.forEach(((item, index) => {
                if (item.querySelector(".menu-button-catalog")) return;
                ...
                menuLink.addEventListener("click", (e => {
                    ...
                    headerMenuPC.classList.add("active");
                    if (headerMenuPCMenus[index]) headerMenuPCMenus[index].classList.add("active");
                }));
            }));
            if (menuButtons && menuButtons.length) menuButtons.forEach((button => {
                button.addEventListener("click", (e => {
                    e.preventDefault();
                    const targetId = button.dataset.target;
                    const targetMenu = document.querySelector(`.header-menu-pc__menu[data-id="${targetId}"]`);
                    if (targetMenu) if (targetMenu.classList.contains("active")) closeAllMenus(); else {
                        closeAllMenus();
                        headerMenuPC.classList.add("active");
            ...
            if (catalogButton) catalogButton.addEventListener("click", (e => {
                e.preventDefault();
                console.log("Клик по кнопке каталога");
                if (catalogMenu && catalogMenu.classList.contains("active")) closeAllMenus(); else {
                    closeAllMenus();
                    headerMenuPC.classList.add("active");
                    if (catalogMenu) {
                        console.log("Активация меню каталога");
                        catalogMenu.classList.add("active");
                    }
                }
            }));
            headerMenuPC.addEventListener("click", (e => {
                const isActiveSubmenuClick = e.target.closest(".header-menu-pc__menu.active");
                if (!isActiveSubmenuClick) closeAllMenus();
            }));
            document.addEventListener("click", (e => {
                if (!e.target.closest(".header-menu-pc") && !e.target.closest(".menu__item") && !e.target.closest(".menu-button") && !e.target.closest(".menu-button-catalog")) closeAllMenus();
            }));
        };
        const scrollToTopButton = () => {
            ...
            const toggleButtonVisibility = () => {
                if (window.pageYOffset > 300) button.classList.add("active"); else button.classList.remove("active");
            };
            const scrollToTop = e => {
                e.preventDefault();
                window.scrollTo({
        ...
        const mobileFiltersAndSort = () => {
            const filterButton = document.querySelector(".actions-products-button--filter");
            const sortButton = document.querySelector(".actions-products-button--sort");
            const sidebar = document.querySelector(".catalog-page__sidebar");
            const productsFilter = document.querySelector(".catalog-page__products-filter");
            const closeButtons = document.querySelectorAll(".catalog-close");
            if (!filterButton || !sidebar || !sortButton || !productsFilter) return;
            const toggleFilter = e => {
                e.preventDefault();
                sidebar.classList.toggle("active");
                if (productsFilter.classList.contains("active")) productsFilter.classList.remove("active");
            };
            const toggleSort = e => {
                e.preventDefault();
            ...
            const handleOutsideClick = e => {
                if (!sidebar.contains(e.target) && !productsFilter.contains(e.target) && !filterButton.contains(e.target) && !sortButton.contains(e.target)) {
                    sidebar.classList.remove("active");
                    productsFilter.classList.remove("active");
                }
            };
        ...
        function initMenuCategories() {
            if (window.innerWidth > 992) {
                console.log("Инициализация меню категорий");
                const menuItems = document.querySelectorAll(".menu-pc__column a[data-subcategory]");
                console.log("Найдено пунктов меню с data-subcategory:", menuItems.length);
                const subcategories = document.querySelectorAll(".subcategory-content");
                console.log("Найдено подкатегорий:", subcategories.length);
                const menuContainers = document.querySelectorAll(".menu-pc-list");
                console.log("Найдено контейнеров меню:", menuContainers.length);
                menuContainers.forEach(((container, index) => {
                    console.log(`Обработка контейнера #${index + 1}`);
                    const firstSubcategory = container.querySelector(".subcategory-content");
                    const firstMenuItem = container.querySelector(".menu-pc__column a[data-subcategory]");
                    console.log("Первая подкатегория:", firstSubcategory ? "найдена" : "не найдена");
                    console.log("Первый пункт меню:", firstMenuItem ? "найден" : "не найден");
                    if (firstSubcategory && firstMenuItem) {
                        console.log("Активация первой подкатегории и пункта меню");
                ...
                menuItems.forEach((item => {
                    item.addEventListener("mouseenter", (() => {
                        const parentMenu = item.closest(".menu-pc-list");
                        console.log("Родительское меню:", parentMenu ? "найдено" : "не найдено");
                        if (!parentMenu) return;
                        const subcategoryId = item.dataset.subcategory;
                        console.log("ID подкатегории:", subcategoryId);
                        const currentMenuItems = parentMenu.querySelectorAll(".menu-pc__column a[data-subcategory]");
                        const currentSubcategories = parentMenu.querySelectorAll(".subcategory-content");
                        console.log("Пунктов меню в текущем контейнере:", currentMenuItems.length);
                        console.log("Подкатегорий в текущем контейнере:", currentSubcategories.length);
                        currentMenuItems.forEach((i => i.classList.remove("active")));
                        currentSubcategories.forEach((s => s.classList.remove("active")));
                        item.classList.add("active");
                        const targetSubcategory = parentMenu.querySelector(`.subcategory-content[data-subcategory-id="${subcategoryId}"]`);
                        console.log("Целевая подкатегория:", targetSubcategory ? "найдена" : "не найдена");
                        if (targetSubcategory) targetSubcategory.classList.add("active");
                    }));
                }));
            }
        }
        function initMobileMenu() {
            ...
            const closeButton = document.querySelector(".header-menu-mobile__close");
            const body = document.body;
            const catalogButton = document.querySelector('.header-menu-mobile__link[data-target="mobile-catalog"]');
            const aboutButton = document.querySelector('.header-menu-mobile__link[data-target="about-company"]');
            console.log("Кнопка каталога в мобильном меню:", catalogButton ? "найдена" : "не найдена");
            console.log("Кнопка о компании в мобильном меню:", aboutButton ? "найдена" : "не найдена");
            const catalogLevel = document.querySelector('.header-menu-mobile__level[data-id="mobile-catalog"]');
            const aboutLevel = document.querySelector('.header-menu-mobile__level[data-id="about-company"]');
            console.log("Уровень каталога в мобильном меню:", catalogLevel ? "найден" : "не найден");
            console.log("Уровень о компании в мобильном меню:", aboutLevel ? "найден" : "не найден");
            let navigationHistory = [];
            if (!mobileMenu) return;
            function toggleMenu() {
                body.classList.toggle("menu-open");
                if (!body.classList.contains("menu-open")) resetLevels();
            }
            function resetLevels() {
                const activeLevels = document.querySelectorAll(".header-menu-mobile__level.active");
                activeLevels.forEach((level => {
                    if (level.dataset.level !== "0") level.classList.remove("active");
                }));
                navigationHistory = [];
            }
            function goToSubmenu(e) {
                e.preventDefault();
                const currentLevel = this.closest(".header-menu-mobile__level");
                const targetId = this.dataset.target;
                console.log("Переход к подменю:", targetId);
                const targetLevel = document.querySelector(`.header-menu-mobile__level[data-id="${targetId}"]`);
                console.log("Целевой уровень:", targetLevel ? "найден" : "не найден");
                if (targetLevel) {
                    if (currentLevel) navigationHistory.push(currentLevel);
            ...
            function initEvents() {
                ...
                document.addEventListener("click", (function(e) {
                    if (e.target.closest(".header-menu-mobile__link[data-target]")) {
                        const link = e.target.closest(".header-menu-mobile__link[data-target]");
                        goToSubmenu.call(link, e);
                    }
                    if (e.target.closest(".header-menu-mobile__back")) {
                        const backButton = e.target.closest(".header-menu-mobile__back");
                        goBack.call(backButton, e);
                    }
        ...
        function initCatalogMenu() {
            console.log("Инициализация меню каталога");
            const catalogMenu = document.getElementById("catalog-menu");
            if (!catalogMenu) {
                console.log("Меню каталога не найдено");
                return;
            }
            console.log("Меню каталога найдено");
            const firstSubcategory = catalogMenu.querySelector(".subcategory-content");
            const firstMenuItem = catalogMenu.querySelector(".menu-pc__column a[data-subcategory]");
            if (firstSubcategory && firstMenuItem) {
                console.log("Активация первой подкатегории и пункта меню в каталоге");
                firstSubcategory.classList.add("active");
                firstMenuItem.classList.add("active");
            }
            const menuItems = catalogMenu.querySelectorAll(".menu-pc__column a[data-subcategory]");
            menuItems.forEach((item => {
                item.addEventListener("mouseenter", (() => {
                    const subcategoryId = item.dataset.subcategory;
                    console.log("Наведение на пункт каталога:", subcategoryId);
                    menuItems.forEach((i => i.classList.remove("active")));
                    const subcategories = catalogMenu.querySelectorAll(".subcategory-content");
                    subcategories.forEach((s => s.classList.remove("active")));
                    item.classList.add("active");
                    const targetSubcategory = catalogMenu.querySelector(`.subcategory-content[data-subcategory-id="${subcategoryId}"]`);
                    if (targetSubcategory) {
                        console.log("Активация подкатегории каталога:", subcategoryId);
        ...
        function addMobileMenuHandlers() {
            ...
            const catalogLevel = document.querySelector('.header-menu-mobile__level[data-id="mobile-catalog"]');
            const aboutLevel = document.querySelector('.header-menu-mobile__level[data-id="about-company"]');
            if (catalogButton && catalogLevel) catalogButton.addEventListener("click", (function(e) {
                e.preventDefault();
                const currentLevel = this.closest(".header-menu-mobile__level");
                if (currentLevel) if (window.navigationHistory) window.navigationHistory.push(currentLevel);
                catalogLevel.classList.add("active");
            }));
            if (aboutButton && aboutLevel) aboutButton.addEventListener("click", (function(e) {
                e.preventDefault();
                const currentLevel = this.closest(".header-menu-mobile__level");
                if (currentLevel) if (window.navigationHistory) window.navigationHistory.push(currentLevel);
                aboutLevel.classList.add("active");
            }));
            const backButtons = document.querySelectorAll(".header-menu-mobile__back");
        ...
        window.addEventListener("load", initMenuCategories);
        window.addEventListener("resize", initMenuCategories);
        const keyServicesAnimation = () => {
            const keyServicesItems = document.querySelectorAll(".key-services__item");
            const keyServicesImage = document.querySelector(".key-services__image img");
            const keyServicesMobile = document.querySelector(".key-services__mob");
            const keyServicesPC = document.querySelector(".key-services__pc");
            if (!keyServicesItems.length) return;
            ...
            const initializeImagePosition = () => {
                if (isMobile() || !keyServicesImage) return;
                const activeItem = document.querySelector(".key-services__pc .key-services__item.active");
                if (activeItem) {
                    const itemClass = Array.from(activeItem.classList).find((cls => cls.startsWith("item-")));
                    if (itemClass && rotateSettings[itemClass] !== void 0) {
                        currentRotation = rotateSettings[itemClass];
                        keyServicesImage.style.transition = "none";
                        keyServicesImage.style.transform = `rotate(${currentRotation}deg)`;
                        void keyServicesImage.offsetWidth;
                        keyServicesImage.style.transition = "transform 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)";
                    }
                }
            };
            const smoothRotate = (element, targetAngle) => {
                if (isMobile() || !element) return currentRotation;
                let diff = ((targetAngle - currentRotation) % 360 + 360) % 360;
            ...
            const showContent = item => {
                const content = item.querySelector(".key-services__item-content");
                if (!content) return;
                content.style.display = "block";
                if (!isMobile() && item.closest(".key-services__pc")) {
            ...
            const setupItemClickHandlers = () => {
                const items = isMobile() ? document.querySelectorAll(".key-services__mob .key-services__item") : document.querySelectorAll(".key-services__pc .key-services__item");
                items.forEach((item => {
                    item.addEventListener("click", (() => {
                        if (item.classList.contains("active")) return;
                        const itemClass = Array.from(item.classList).find((cls => cls.startsWith("item-")));
                        items.forEach((el => {
                            el.classList.remove("active");
                            el.classList.remove("expanded");
                            const content = el.querySelector(".key-services__item-content");
                            if (content) content.style.display = "none";
                        }));
                        item.classList.add("active");
                        if (!isMobile() && keyServicesImage && itemClass && rotateSettings[itemClass] !== void 0) currentRotation = smoothRotate(keyServicesImage, rotateSettings[itemClass]);
                        showContent(item);
                    }));
                }));
            };
            const setDefaultActiveItem = () => {
                const container = isMobile() ? keyServicesMobile : keyServicesPC;
                if (!container) return;
                const items = container.querySelectorAll(".key-services__item");
                if (items.length >= 3) {
                    ...
                    if (!isMobile() && keyServicesImage) {
                        const itemClass = Array.from(thirdItem.classList).find((cls => cls.startsWith("item-")));
                        if (itemClass && rotateSettings[itemClass] !== void 0) {
                            currentRotation = rotateSettings[itemClass];
                            keyServicesImage.style.transform = `rotate(${currentRotation}deg)`;
                        }
                    }
                    const content = thirdItem.querySelector(".key-services__item-content");
                    if (content) {
                        content.style.display = "block";
                        content.style.opacity = "1";
                        content.style.transform = "translateY(0)";
            ...
            const handleResize = () => {
                ...
                if (keyServicesImage) if (isMobile()) keyServicesImage.style.transform = "rotate(0deg)"; else {
                    const activeItem = document.querySelector(".key-services__pc .key-services__item.active");
                    if (activeItem) {
                        const itemClass = Array.from(activeItem.classList).find((cls => cls.startsWith("item-")));
                        if (itemClass && rotateSettings[itemClass] !== void 0) keyServicesImage.style.transform = `rotate(${rotateSettings[itemClass]}deg)`;
                    }
                }
        ...
        document.addEventListener("DOMContentLoaded", keyServicesAnimation);
        document.addEventListener("DOMContentLoaded", (function() {
            const path = document.getElementById("circle-path");
            const circle = document.getElementById("moving-circle");
            const items = document.querySelectorAll(".benefits-main__item");
            if (!path || !circle) {
                console.error("Элементы пути или круга не найдены");
                return;
            }
            ...
            function animate() {
                ...
                if (position >= pathLength) direction = -1; else if (position <= 0) direction = 1;
                const point = getPointAtLength(position);
                circle.style.transform = `translate(${point.x}px, ${point.y}px)`;
                requestAnimationFrame(animate);
            }
            const startPoint = getPointAtLength(0);
            circle.style.transform = `translate(${startPoint.x}px, ${startPoint.y}px)`;
            animate();
            items.forEach(((item, index) => {
                item.addEventListener("click", (function() {
                    ...
                    autoAnimation = false;
                    const targetPosition = index / (items.length - 1) * pathLength;
                    const point = getPointAtLength(targetPosition);
                    animateCircleTo(point);
                }));
            }));
            function animateCircleTo(targetPoint) {
                ...
                function step(time) {
                    ...
                    const x = currentX + (targetPoint.x - currentX) * easeProgress;
                    const y = currentY + (targetPoint.y - currentY) * easeProgress;
                    circle.style.transform = `translate(${x}px, ${y}px)`;
                    if (progress < 1) requestAnimationFrame(step); else setTimeout((() => {
                        position = targetPoint.x / pathLength * pathLength;
                        autoAnimation = true;
                        animate();
                    }), 3e3);
                }
